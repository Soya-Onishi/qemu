                                          ...
                                          bool force)
{
    ...
    if (!force && !timer_pending(s->outport_timer)) {
        timer_mod(s->outport_timer,
                  qemu_clock_get_ns(QEMU_CLOCK_HOST) + 1000 * 1000);
    }
    ...
    if (transmit_data) {
        ...
        IPCSignalPayload *payloads = g_new(IPCSignalPayload, length);
        ...
        qemu_chr_fe_write_all(&s->port_chardev, (uint8_t *)payloads,
                              length * sizeof(IPCSignalPayload));
        g_free(payloads);
    }
}
...
static void rl78_dynamic_distribute_payloads(void *opaque)
{
    ...
    for (GList *it = payloads; it != NULL; it = it->next) {
        IPCSignalPayload *payload = it->data;
 
        uint64_t port_index = GPOINTER_TO_UINT(g_hash_table_lookup(
            s->inport_key_table, GUINT_TO_POINTER(payload->port_id)));
...
static void rl78_dynamic_init(MachineState *machine)
{
    ...
    if (!object_class_dynamic_cast(mcuclass, TYPE_RL78G23_MCU)) {
        error_report("MCU class %s is not a RL78 G23 MCU",
    ...
    if (machine_port) {
        ...
        qemu_chr_fe_set_handlers(
            &s->port_chardev, rl78_dynamic_port_can_receive,
